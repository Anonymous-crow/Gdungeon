...
class Entity {
      public:
	//! Default Constructor
	Entity() : hp{5}, hpCap{5}, name{""}, id{""} {};

	/// @brief The standard Entity constructor.
	/// Sets an int hp, and strings name and id.
	Entity(int newHp, const std::string& newName, const std::string& newID)
	    : hp(newHp), hpCap(newHp), name(newName), id(newID) {};
...
class Player : public Entity {
	...
	Player(Catalog* newCardCatalogPtr)
	    : energy{3}, handsize{5}, cardCatalogPtr(newCardCatalogPtr),
	      Entity() {}
...
class Enemy : public Entity {
	...
	Enemy(const std::string& newName,
	    ...
	    : Entity(newHp, newName, newID), cardCatalogPtr(newCardCatalogPtr),
	      size{newSize}, rangeType{newRange}, A(newA), B(newB) {
		initIntentionDeck();
	}
	...
	void addIntent(const std::string& intentID, int copies = 1);
...
template <class T> struct ptrIterator {
	...
	inline ptrIterator<T> operator+(difference_type rhs) {
	...
	inline friend ptrIterator<T> operator+(difference_type lhs,
					       ptrIterator<T>& rhs) {
	...
	inline friend bool operator==(const ptrIterator<T>& a,
				      const ptrIterator<T>& b) {
...
class Party : ptrIterator<Player> {
	...
	iterator remove(iterator itb,
      ...
      private:
	Player* partyList[4]{nullptr, nullptr, nullptr, nullptr};
	Catalog* cardCatalogPtr{nullptr};
};
