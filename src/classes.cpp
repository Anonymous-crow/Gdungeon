...
std::string Player::getDeckString() const {
	...
	bool first{false};
	for (std::map<std::string, int>::const_iterator i = deckMap.begin();
	     i != deckMap.end();
	     ++i) {
		out << "\t   ";
		if (!first) {
...
void Player::addCardToDeck(const std::string& cardID, int copies) {
	...
	if (found == 0) {
		...
		if (newCard != nullptr) {
			...
			std::cerr << "Could not load card " << cardID
				  << "! Card not found." << std::endl;
		}
	} else if (newCard->copies > found) {
...
void Player::removeCardFromDeck(const std::string& cardID, int toRemove) {
	int deleted{0};
...
Party::iterator Party::erase(Party::iterator itStart, Party::iterator itEnd) {
	if (itStart == itEnd) {
		return itStart;
	}
	Party::iterator end = this->end();
	...
	for (Party::iterator it = itStart; (it != itEnd) && (it != end); ++it) {
		delete *it;
		*it = nullptr;
	}
	if (itEnd == end) {
...
void Party::addMember(int newHp,
	...
	if (idNotFound && nullPos < 4) {
		partyList[nullPos] =
		    new Player(newHp, newName, newID, cardCatalogPtr);
	}
}
